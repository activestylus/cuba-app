module Rack
  class CommonLogger
    ...
    def log(env, status, header, began_at)
      ...
      logger.write Chroma::Format.request(env, status, now, elapsed)
    end
  end
  class MemoryLogger
...
module Chroma
  ...
  class Format
    ...
    def self.logger(severity, time, progname, message)
      l = []
      l << "[#{ time.strftime(timestamp) }]".dye(:slate)
      l << "[#{ ("%5s" % [severity]) }]"
      l << progname
      l << message
      l << "\n"
    ...
    # Response Prompt
    def self.request(env, status, now, elapsed)
      code    = status.to_s[0..3]
      f       = Chroma::Utils.flag(code)
      stamp   = now.strftime(Chroma::Format.timestamp)
      method  = env["REQUEST_METHOD"]
      path    = env["PATH_INFO"]
      elapsed = "#{ (elapsed * 1000).round(4) }ms"
      l =  []
      l << "[#{stamp}]".dye("#{f}_plain".to_sym)
      l << code.dye("#{f}_code".to_sym)
      l << method.dye("#{f}_method".to_sym)
      l << path.dye("#{f}_path".to_sym)
      l << elapsed.dye("#{f}_plain".to_sym) + "\n"
      l.join(' ')
    end

    # Watch Memory Bloat
    def self.memory(sym, diff)
      l =  "RAM: ".dye(:memory_text)
      l << "#{(Chroma::Utils.memory_usage).to_s.dye(:memory_size)}"
      l << "K (".dye(:memory_text)
      l << "#{sym}#{diff}".dye(:memory_size)
      l << "K)".dye(:memory_text)
      l
    end
  ...
  module Palettes
    ...
    def self.print_hex
      prompt = ""
      self.hex.each_slice(3).to_a.each do |row|
        over100 = row.find_all {|x| x[1] > 100}.empty?
        join = over100 ? ",  " : ", "
        s = ""
        s << row.map do |x|
          "\"#{x[0]}\" => #{x[1]}".dye(x[1])
